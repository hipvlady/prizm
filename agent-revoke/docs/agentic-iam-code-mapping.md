...
| Least Privilege | `src/core/types.py::DelegationPolicy` + `src/core/exceptions.py::BudgetExceededError` | Prevents delegating exhausted op budgets (`RemainingOpsPropagationError` alias remains supported) |
...
| Scope attenuation | `delegate_capability(... attenuated_scope=...)` subset check | Enforced at delegation time with `ScopeViolationError` (`ScopeAttenuationError` retained as compatible naming in types) |
